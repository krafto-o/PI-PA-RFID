...
def abrir_ventana_consulta():
    ...
    ventana_consulta.configure(bg=bg_color)
    ...
    entryBusqueda = ttk.Entry(frame_busqueda, width=40)
    ...
    lblCount = ttk.Label(ventana_consulta, text="Total: 0 registros")
...
def procesar_linea(linea):
    linea = linea.strip()
    if linea == "READY":
        ventana_principal.after(0, lambda: lbConexion.config(text="Conectado"))
        return
...
def btnConectar_Click():
    ...
    if modo_test:
        ...
        lbConexion.config(text="Conectado (TEST)")
        test_simulator.start_simulator(procesar_linea)
        return
    ...
    try:
        ...
        btnConectar.config(state=tk.DISABLED)
        btnDesconectar.config(state=tk.NORMAL)
        lbConexion.config(text="Conectando...")
        ...
        lbConexion.config(text="Desconectado")
...
def btnDesconectar_Click():
    ...
    if not modo_test and arduino.is_open:
        arduino.close()

    lbConexion.config(text="Desconectado")
    lbEstado.config(text="Esperando...")
    lbUUID.config(text="UUID: ---")


def btnSimular_Click():
    linea = test_simulator.generar_linea()
    procesar_linea(linea)
...
ventana_principal = ThemedTk(theme="equilux")
...
frame_header = ttk.Frame(ventana_principal)
frame_header.pack(pady=10)

lbConexion = ttk.Label(frame_header, text="Desconectado", font=("Arial", 12, "bold"))
lbConexion.pack()

lbEstado = ttk.Label(ventana_principal, text="Esperando...", font=("Arial", 16, "bold"))
lbEstado.pack(pady=10)

lbUUID = ttk.Label(ventana_principal, text="UUID: ---", font=("Arial", 11))
lbUUID.pack()
lbLog = ttk.Label(ventana_principal, text="", font=("Arial", 9))
lbLog.pack(pady=5)
