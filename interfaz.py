...
def abrir_ventana_consulta():
    ...
    btnBuscar = tk.Button(frame_busqueda, text="Buscar", command=on_buscar, bg="#3498db", fg="white")
    btnBuscar.pack(side=tk.LEFT, padx=5)

    btnLimpiar = tk.Button(frame_busqueda, text="Limpiar", command=lambda: [entryBusqueda.delete(0, tk.END), on_rango_change()], bg="#95a5a6", fg="white")
    btnLimpiar.pack(side=tk.LEFT, padx=5)

    if modo_test:
        btnPoblarDB = tk.Button(frame_busqueda, text="Poblar DB", command=on_poblar_db, bg="#e67e22", fg="white")
    ...
    btnCerrar = tk.Button(frame_cerrar, text="Cerrar", command=ventana_consulta.destroy, bg="#c0392b", fg="white", width=12)
    btnCerrar.pack()
...
def btnConectar_Click():
    ...
    if modo_test:
        ...
        lbConexion.config(text="Conectado (TEST)", fg="orange")
...
if modo_test:
    btnSimular = tk.Button(frame_botones, text="Simular", command=btnSimular_Click, width=12, bg="#f39c12", fg="white")
